...
- **Frontend**: `App` opens on the warm-up (`Warmup.tsx`) with a Warm-up/Practice tab; the editor is now the `Practice` component. The runner walks the set, re-queues a wrong rep to the end (bounded by `MAX_REQUEUES`), and keys its rep-load effect on the current id+position (not the whole queue) so re-queueing does not reload and wipe the feedback. Shared fetch helpers live in `src/api.ts`.
